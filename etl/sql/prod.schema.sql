...
(
	npi integer
		constraint provider_performance_providers_npi_fk
			references cms.providers,
	mcare_participation_indicator text,
	place_of_service text,
	hcpcs_code text
		constraint provider_performance_cms_svcs_hcpcs_code_fk
...
(
	hcpcs_code text
		constraint service_provider_performance_svcs_hcpcs_code_fk
			references cms.services,
	npi integer
...
CREATE SEQUENCE cms.service_provider_performance_summary_type_id_seq NO MINVALUE NO MAXVALUE NO CYCLE;


CREATE TABLE cms.service_provider_performance_summary_type
(
    id integer DEFAULT nextval('cms.service_provider_performance_summary_type_id_seq'::regclass) PRIMARY KEY NOT NULL,
    slug varchar(20),
    description text NOT NULL
);
CREATE UNIQUE INDEX service_provider_performance_summary_type_slug_uindex ON cms.service_provider_performance_summary_type (slug);
ALTER SEQUENCE cms.service_provider_performance_summary_type_id_seq OWNED BY cms.service_provider_performance_summary_type.id;
...
CREATE OR REPLACE VIEW cms.live_service_provider_performance AS SELECT performance.hcpcs_code,
    performance.npi,
...
CREATE OR REPLACE VIEW cms.live_service_performance AS SELECT performance.hcpcs_code,
    providers.entity_type,
...
CREATE OR REPLACE VIEW cms.live_service_provider_performance_summary_overall AS
SELECT
		1 AS summary_type,
		performance.npi,
...
CREATE OR REPLACE VIEW cms.live_service_provider_performance_summary_drug_yes AS SELECT 2 AS summary_type,
    performance.npi,
    performance.entity_type,
    count(performance.hcpcs_code) AS ttl_hcpcs_code,
    sum(performance.n_of_svcs) AS ttl_n_of_svcs,
...
CREATE OR REPLACE VIEW cms.live_service_provider_performance_summary_drug_no AS SELECT 3 AS summary_type,
    performance.npi,
    performance.entity_type,
    count(performance.hcpcs_code) AS ttl_hcpcs_code,
    sum(performance.n_of_svcs) AS ttl_n_of_svcs,
